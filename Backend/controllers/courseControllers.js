...
const getAllCourses = async (req, res, next) => {
  try {
    ...
    return next(
      new AppError(
        "Some kind of error occured.Cannot fetch the courses right now",
        500
      )
    );
  }
...
const updateCourse = async (req, res, next) => {
  try {
    ...
    const course = await Course.findByIdAndUpdate(
      ...
      { $set: req.body },
      { runValidators: true }
    ); // find the course by id and overwrite it based on the data recieved from the request.Run validator ensures if the recieved data is correct
    // console.log(course);
    if (!course) {
